...
class TTT1 extends JFrame implements ItemListener, ActionListener {// interface
	int i, j, ii, jj, x, y, yesnull;
	int a[][] = { { 10, 1, 2, 3, 11 }, { 10, 1, 4, 7, 11 }, { 10, 1, 5, 9, 11 }, { 10, 2, 5, 8, 11 },
			{ 10, 3, 5, 7, 11 }, { 10, 3, 6, 9, 11 }, { 10, 4, 5, 6, 11 }, { 10, 7, 8, 9, 11 } }; // this array is
																									// tracking the user
																									// clicked buttons
																									// so that it can
																									// check which user
	...
	public void actionPerformed(ActionEvent e) {
		...
		boolean draw = true; // user code for showing draw option
		...
		for (i = 0; i <= 7; i++) {
			...
			if ((icon1 == icon2) && (icon2 == icon3) && (icon1 != null)) {
				if (icon1 == ic1) {
					...
					JOptionPane.showMessageDialog(TTT1.this, UserManagement.userName + " won! Click reset"); // UserManagement.userName
					draw = false;
					break;
				} else if (icon1 == ic2) {
					...
					JOptionPane.showMessageDialog(TTT1.this, "Oposite player won! Click reset");
					draw = false;
					break;
				}
		...
		if (draw == true) {
			int stillBlankButtonForClick = 0;
			for (i = 0; i <= 8; i++) {
				if (b[i].getIcon() == null) { // if all icon are not set, then there will be blank option for clicking.
					stillBlankButtonForClick = 1;
				}
			}

			// if there is no blank button then this will execute.
			if (stillBlankButtonForClick == 0) {
				JOptionPane.showMessageDialog(TTT1.this, "Match Draw ! Click reset");
			}
	...
	public static void main(String[] args) {
		new TTT1(); // calling constructor of this class
	}// eof main
}// eof class